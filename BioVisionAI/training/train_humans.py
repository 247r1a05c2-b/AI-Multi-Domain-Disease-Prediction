"""
BioVision AI — Human Skin Disease Classification Training Script
...
DATASET_DIR  = os.path.join(os.path.dirname(os.path.dirname(__file__)), "datasets", "humans")
OUTPUT_MODEL = os.path.join(os.path.dirname(os.path.dirname(__file__)), "saved_models", "human_model.h5")
IMAGE_SIZE   = (224, 224)
BATCH_SIZE   = 16
EPOCHS       = 30
LEARNING_RATE = 1e-4
...
def train():
    ...
    logger.info("=== BioVision AI — Human Skin Model Training ===")
    ...
    with open(os.path.join(charts_dir, "human_training_history.png"), "wb") as f:
        f.write(training_history_png(merged))
    with open(os.path.join(charts_dir, "human_confusion_matrix.png"), "wb") as f:
        f.write(confusion_matrix_png(cm, target_names))

    logger.info(f"Model saved: {OUTPUT_MODEL}")
    logger.info("=== Human training complete! ===")
